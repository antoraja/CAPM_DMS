...
const onUploadAttachment = async (request, next) => {
        try {      
            console.log("request.headers",request.headers.mimetype)
               ...
               //const updateCount = await UPDATE(attachmentItems).set({ attachmentData }).where ({attachID: request.data.attachID}); 
             ...
             // const updateCount = await UPSERT(attachmentItems).set({ attachmentData }).where`attachID = ${request.data.attachID}`;
              //.where ({attachID: request.data.attachID});
